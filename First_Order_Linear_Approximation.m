function [kv_bar,lt_bar] = First_Order_Linear_Approximation(N_v,M_t,v_max,r_v,r_t)
% N_v = 3;   % Doppler domain virtual sampling grid
% M_t = 3;   % delay domain virtual sampling grid
kv_bar = ones(N_v * M_t,1);
lt_bar = ones(N_v * M_t,1);
% k_v_init = [0:t_max/Nv:t_max];
% l_r_init = [-v_max:2*v_max/Mt:v_max];
    for kk = 0:N_v-1
        for ll = 1:M_t
            if (r_v == 0.8)
                kv_bar(kk*M_t+ll) = (kk + 0.25) * r_v - v_max;
            else
                kv_bar(kk*M_t+ll) = (kk + 0.5) * r_v - v_max;
            end
            lt_bar(kk*M_t+ll) = (ll - 0.5) * r_t;
        end
    end
