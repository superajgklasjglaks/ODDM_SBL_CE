...
N_fram = 3;
NMSE_count_trd = zeros(length(SNR_dB),1);
NMSE_count_1D = zeros(length(SNR_dB),1);
NMSE_count_2D = zeros(length(SNR_dB),1);
NMSE_count_1D_o = zeros(length(SNR_dB),1);
NMSE_count_2D_o = zeros(length(SNR_dB),1);
NMSE_count_omp = zeros(length(SNR_dB),1);
NMSE_CRLB = zeros(length(SNR_dB),1);
...
for ifram = 1:N_fram
    ...
    for iesn0 = 1:length(SNR_dB)
        ...
        [h_trd,k_v_trd,l_t_trd] = traditional_impulse(sqrt(1000)/(Kp*Lp),y_trunc,k_max,l_max,sigma_2_p(iesn0));
        [h_hat_1D,k_v_hat_1D,l_t_hat_1D,virtual_size, Phi, delta] = CE_1D_SBL(sqrt(1000)/(Kp*Lp),k_max+Kp,Lp,M,N,N_T,M_T,y_T,r_v,r_t,k_max,l_max,on_flag);
        %[h_hat_1D_o,k_v_hat_1D_o,l_t_hat_1D_o,~] = CE_1D_SBL(sqrt(1000)/(Kp*Lp),k_max+Kp,Lp,M,N,N_T,M_T,y_T,r_v,r_t,k_max,l_max,~on_flag); 
        [H_opt_2D,k_v_opt_2D,l_t_opt_2D,~] = CE_2D_SBL(sqrt(1000)/(2*Kp*Lp),k_max+Kp,2,Lp,M,N,N_T,M_T,y_trunc,r_v,r_t,k_max,l_max,on_flag);
        %[H_opt_2D_o,k_v_opt_2D_o,l_t_opt_2D_o,~] = CE_2D_SBL(sqrt(1000)/(2*Kp*Lp),k_max+Kp,2,Lp,M,N,N_T,M_T,y_trunc,r_v,r_t,k_max,l_max,~on_flag);
        [h_hat_omp, omp_index, k_v_omp, l_t_omp] = OMP(sqrt(1000)/(Kp*Lp),k_max+Kp,Lp,M,N,N_T,M_T,y_T,r_v,r_t,k_max,l_max);
        ...
        for kk = 0:(N-1)
            for ll = 1:M
                ...
                h_w_omp = sum(Sampling_Function_v(N,kk+1,1,k_v_omp) .* Sampling_Function_t(M,ll,1,l_t_omp) .* h_hat_omp .* exp(-2i*pi.*k_v_omp.*l_t_omp/N/M));
                h_w_hat_trd = sum(Sampling_Function_v(N,kk+1,1,k_v_trd) .* Sampling_Function_t(M,ll,1,l_t_trd) .* h_trd .* exp(-2i*pi.*k_v_trd.*l_t_trd/N/M));
                ...
                % h_w_hat_1D_o = sum(Sampling_Function_v(N,kk+1,1,k_v_hat_1D_o) .* Sampling_Function_t(M,ll,1,l_t_hat_1D_o) .* h_hat_1D .* exp(-2i*pi.*k_v_hat_1D_o.*l_t_hat_1D_o/N/M));
                % h_w_hat_2D_o = sum(Sampling_Function_v(N,kk+1,1,k_v_opt_2D_o) .* Sampling_Function_t(M,ll,1,l_t_opt_2D_o) .* H_opt_2D .* exp(-2i*pi.*k_v_opt_2D_o.*l_t_opt_2D_o/N/M));
                NMSE_nume_trd = NMSE_nume_trd + abs(h_w - h_w_hat_trd).^2;
                NMSE_nume_1D = NMSE_nume_1D + abs(h_w - h_w_hat_1D).^2;
                NMSE_nume_2D = NMSE_nume_2D + abs(h_w - h_w_hat_2D).^2;
                % NMSE_nume_1D_o = NMSE_nume_1D_o + abs(h_w - h_w_hat_1D_o).^2;
                % NMSE_nume_2D_o = NMSE_nume_2D_o + abs(h_w - h_w_hat_2D_o).^2;
                NMSE_nume_omp = NMSE_nume_omp + abs(h_w - h_w_omp).^2;
                NMSE_deno = NMSE_deno + abs(h_w)^2;
            end 
        end
        J = zeros(M_T*N_T, 2*k_max*l_max/r_v/r_t);
        for m=1:M_T
            for n=1:N_T
        ...
        C_h = (sigma_2(iesn0)^(-1) * (Phi' * Phi) + delta^(-1))^(-1);
        C_alpha = J * C_h * J.';
        NMSE_CRLB(iesn0) = trace(abs(C_alpha));
        NMSE_count_trd(iesn0) = NMSE_count_trd(iesn0) + NMSE_nume_trd / (NMSE_deno * N_fram);
        ...
        % NMSE_count_1D_o(iesn0) = NMSE_count_1D_o(iesn0) + NMSE_nume_1D_o / (NMSE_deno * N_fram);
        % NMSE_count_2D_o(iesn0) = NMSE_count_2D_o(iesn0) + NMSE_nume_2D_o / (NMSE_deno * N_fram);
...
%%
NMSE_dB_trd = 10 * log10(NMSE_count_trd);
NMSE_dB_1D = 10 * log10(NMSE_count_1D);
NMSE_dB_2D = 10 * log10(NMSE_count_2D);
% NMSE_dB_1D_o = 10 * log10(NMSE_count_1D_o);
% NMSE_dB_2D_o = 10 * log10(NMSE_count_2D_o);
...
legend('Traditional impulse','OMP','1D off-grid','4D off-grid', 'CRLB');
xlabel('SNR(dB)')
ylabel('NMSE(dB)')
