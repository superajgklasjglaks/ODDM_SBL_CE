...
SNR_p_fixed = SNR_fixed * 1e5;
sigma_2_p_fixed = sigma_2_fixed / 1e5;

%% 初始化仿真误差计数变量

N_fram = 10;  % 仿真帧数
NMSE_count_1D = zeros(length(k_max_range),1);
NMSE_count_2D = zeros(length(k_max_range),1);
NMSE_count_hierarchical_1D = zeros(length(k_max_range),1);
NMSE_count_hierarchical_2D = zeros(length(k_max_range),1);
NMSE_count_OMP = zeros(length(k_max_range),1);
NMSE_count_traditional = zeros(length(k_max_range),1);
NMSE_CRLB = zeros(length(k_max_range),1);  % CRLB理论下界
err_ber = zeros(length(k_max_range),1);
H_mtx = zeros(M*N,M*N);

fprintf('\n=== 六种算法性能比较开始 (k_max变化分析) ===\n');
fprintf('仿真参数：N=%d, M=%d, P=%d, N_fram=%d, SNR=%ddB\n', N, M, P, N_fram, SNR_dB_fixed);
fprintf('k_max变化范围：%d到%d，步长为%d\n', min(k_max_range), max(k_max_range), k_max_range(2)-k_max_range(1));
fprintf('\n算法分辨率参数：\n');
...
for ifram = 1:N_fram
    ...
    for ik_max = 1:length(k_max_range)
        k_max = k_max_range(ik_max);
        
        % 根据当前k_max计算相关参数
        t_max = l_max / M / delta_f;
        v_max = k_max / N / T;
        
        fprintf('Frame %d/%d, k_max = %d\n', ifram, N_fram, k_max);
        ...
        %% 计算CRLB理论下界
         ...
         % 计算协方差矩阵
         C_h = (sigma_2_fixed^(-1) * (Phi_1D' * Phi_1D) + delta_a^(-1))^(-1);
         % C_h = sigma_2_fixed * (Phi_1D' * Phi_1D)^(-1);
         C_alpha = J * C_h * J.';
         NMSE_CRLB(ik_max) = trace(abs(C_alpha));
        ...
        NMSE_count_1D(ik_max) = NMSE_count_1D(ik_max) + NMSE_nume_1D / (NMSE_deno * N_fram);
        NMSE_count_2D(ik_max) = NMSE_count_2D(ik_max) + NMSE_nume_2D / (NMSE_deno * N_fram);
        NMSE_count_hierarchical_1D(ik_max) = NMSE_count_hierarchical_1D(ik_max) + NMSE_nume_hierarchical_1D / (NMSE_deno * N_fram);
        NMSE_count_hierarchical_2D(ik_max) = NMSE_count_hierarchical_2D(ik_max) + NMSE_nume_hierarchical_2D / (NMSE_deno * N_fram);
        NMSE_count_OMP(ik_max) = NMSE_count_OMP(ik_max) + NMSE_nume_OMP / (NMSE_deno * N_fram);
        NMSE_count_traditional(ik_max) = NMSE_count_traditional(ik_max) + NMSE_nume_traditional / (NMSE_deno * N_fram);
    end
end
...
plot(k_max_range,NMSE_dB_1D,'-v', 'Color', colors(3,:),'LineWidth',2,'MarkerSize',8);
hold on
plot(k_max_range,NMSE_dB_2D,'-*', 'Color', colors(2,:),'LineWidth',2,'MarkerSize',8);
plot(k_max_range,NMSE_dB_hierarchical_1D,'-s', 'Color', colors(1,:),'LineWidth',2,'MarkerSize',8);
plot(k_max_range,NMSE_dB_hierarchical_2D,'-o', 'Color', colors(4,:),'LineWidth',2,'MarkerSize',8);
plot(k_max_range,NMSE_dB_OMP,'-d', 'Color', colors(6,:),'LineWidth',2,'MarkerSize',8);
plot(k_max_range,NMSE_dB_traditional,'-x', 'Color', colors(7,:),'LineWidth',2,'MarkerSize',8);
plot(k_max_range,NMSE_dB_CRLB,'-^', 'Color', colors(5,:),'LineWidth',2,'MarkerSize',8);
grid on

% 添加四周边框
box on;
set(gca, 'LineWidth', 1.5);  % 设置边框线宽
set(gca, 'XColor', 'black'); % 设置X轴颜色
set(gca, 'YColor', 'black'); % 设置Y轴颜色

legend('1D SBL', '2D SBL', '分层1D SBL', '分层2D SBL', 'OMP', 'Traditional Impulse', 'CRLB下界', 'Location', 'best');
xlabel('k_{max}', 'FontSize', 12);
ylabel('NMSE (dB)', 'FontSize', 12);
title(sprintf('六种算法信道估计性能比较 (SNR = %d dB)', SNR_dB_fixed), 'FontSize', 14);
set(gca, 'FontSize', 11);

%% 结果分析和显示
fprintf('\n=== 六种算法性能比较结果 (k_max变化分析) ===\n');
fprintf('k_max\t1D SBL\t\t2D SBL\t\t分层1D SBL\t分层2D SBL\tOMP\t\t\tTraditional\tCRLB下界\n');
fprintf('-----\t------\t\t------\t\t----------\t----------\t----------\t----------\t--------\n');
for i = 1:length(k_max_range)
    fprintf('%d\t\t%.2f\t\t%.2f\t\t%.2f\t\t%.2f\t\t%.2f\t\t%.2f\t\t%.2f\n', k_max_range(i), NMSE_dB_1D(i), NMSE_dB_2D(i), NMSE_dB_hierarchical_1D(i), NMSE_dB_hierarchical_2D(i), NMSE_dB_OMP(i), NMSE_dB_traditional(i), NMSE_dB_CRLB(i));
end
